...
from fontFeatures import Attachment, Chaining, FontFeatures, Positioning, Routine, Substitution, ValueRecord
...
class MalayalamFont(Font):
    ...
    def build_latin_ligatures(self):
        ...
        for ligature in ligatures:
            sub = Substitution(
                [[SVGGlyph.get_glyph_name(l)] for l in ligature], replacement=[[SVGGlyph.get_glyph_name(ligature)]]
            )
            rules.append(sub)
        ...
        for script in self.options.glyphs.kern:
            ...
            for kern_def in self.options.glyphs.kern[script]:
                ...
                for l, r in product(lhs, rhs):
                    rules.append(Positioning([[l], [r]], [ValueRecord(xAdvance=xAdvance), ValueRecord()]))
    ...
    def build_chillus(self):
        feature = "akhn"
        name = "zwj_chillus"
        chillus = ["ന്\u200d", "ര്\u200d", "ല്\u200d", "ള്\u200d", "ണ്\u200d", "ഴ്\u200d", "ക്\u200d"]

        rules = []
        for chillu in chillus:
            rules.append(
                Substitution(
                    [[SVGGlyph.get_glyph_name(l)] for l in chillu], replacement=[[SVGGlyph.get_glyph_name(chillu)]]
                )
            )
    ...
    def build_cons_la(self):
        ...
        for conjunct in self.get_glyphs_from_named_classes("ML_LA_CONJUNCTS"):
            rules.append(
                Substitution(
                    [[SVGGlyph.get_glyph_name(l)] for l in conjunct], replacement=[[SVGGlyph.get_glyph_name(conjunct)]]
                )
            )
        routine = Routine(rules=rules, name=name, languages=LANGUAGE_MALAYALAM)
        self.fontFeatures.addFeature(feature, [routine])
    ...
    def build_conjuncts(self):
        ...
        for conjunct in self.get_glyphs_from_named_classes("ML_CONS_CONJUNCTS"):
            ...
            if conjunct_glyph_name not in self:
                continue
            sub = Substitution([[SVGGlyph.get_glyph_name(l)] for l in conjunct], replacement=[[conjunct_glyph_name]])
        ...
        for base in base_for_u:
            ...
            if uu_glyph_name not in self and uu_glyph_name not in self.get_glyph_names_from_named_classes(
                ...
                if base_glyph_name in self.salts:
                    ...
                    self.buildComposite(uu_glyph_name_alt, None, [self.salts[base_glyph_name][0], "u_drop_sign"])
